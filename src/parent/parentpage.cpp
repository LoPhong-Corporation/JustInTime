...
ParentPage::ParentPage(QWidget *parent) : QWidget(parent)
{
    // ---------------- Mời con ----------------
    ...
    connect(inviteBtn, &QPushButton::clicked, this, &ParentPage::onInvite);
    ...
    m_childrenTable->setAlternatingRowColors(true);
    m_childrenTable->verticalHeader()->setVisible(false);
    m_childrenTable->setMinimumHeight(140);

    connect(
        m_childrenTable, &QTableWidget::itemSelectionChanged,
        this, &ParentPage::onChildSelectionChanged
    );
    ...
    m_limitsTable->setAlternatingRowColors(true);
    m_limitsTable->verticalHeader()->setVisible(false);
    m_limitsTable->setMinimumHeight(140);
    ...
    saveLimitBtn->setProperty("accent", true);

    connect(saveLimitBtn, &QPushButton::clicked, this, &ParentPage::onSaveLimit);
    connect(deleteLimitBtn, &QPushButton::clicked, this, &ParentPage::onDeleteLimit);
    ...
    // ---------------- Tổng thể (cuộn được - nội dung khá dài) ----------------

    auto *heading = new QLabel(tr_("pdlg.title"), this);
    heading->setStyleSheet("font-size: 16px; font-weight: 600;");

    auto *content = new QWidget;
    auto *contentLayout = new QVBoxLayout(content);
    contentLayout->addWidget(inviteBox);
    contentLayout->addWidget(childrenBox);
    contentLayout->addWidget(limitsBox);
...
void ParentPage::refresh()
{
    refreshChildren();
}

void ParentPage::refreshChildren()
{
...
QString ParentPage::selectedChildUserId() const
{
...
void ParentPage::onChildSelectionChanged()
{
    refreshLimits();
}

void ParentPage::refreshLimits()
{
